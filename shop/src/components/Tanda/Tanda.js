...
export default function Tanda() {
    ...
    return (
        <div className="container-tanda">
            <div className="container-logo">
                <img className="logoTanda" src={logo} alt=""></img>
            </div>

            <div className="container-tanda-turn">
            <h2 id="id_titulo_tanda" ></h2>
        
                <div>
                    <p className="p-turn">actual:</p>
                    <textarea readOnly id="id_texto_n_actual" rows="1" cols="3" ></textarea>
                </div>
            
                <div>
                    <p className="p-turn">total:</p>
                    <textarea readOnly id="id_texto_n_total" rows="1" cols="3" wrap="hard"> </textarea>
                </div>

                <div>
                    <button className="button-tanda" id="btn_pause_start_tanda" onClick={handler_start_pause}>Start</button>
                    <button className="button-tanda" id="btn_stop" onClick={handler_stop}>Stop</button>
                </div>

                <div>
                    <button className="button-tanda" id="btn_anterior" onClick={handler_anterior}>Anterior</button>
                    <button className="button-tanda" id="btn_siguiente" onClick={handler_siguiente}>Siguiente</button>
                
                </div>
            <Link className="go-panel" to="/panel">Volver al panel</Link>
        </div>
    </div> 
    )
}
